...
By default the srsSerial plugin hides messages that contain nothing but carrier detect (in practice ~90% of real traffic is COR-only noise); messages carrying anything more — and the 100-second port-state heartbeats — pass through complete. Tune this per collector with `"extendedOptions": { "suppress": [...] }` — an empty list shows everything, and any radio/port state name (e.g. `"LINK"`, `"LOOPBACK"`) can be added to the list. Equivalent Arduino sketches for driving a real serial port from a microcontroller live in **sims/arduino/**.
