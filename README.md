...
A software fleet of simulated UDP devices is also available for development: `npm run sim:udp` (add `--secret <hex32>` for signed traffic, `--once` for a single burst). For load testing there is a rate-controlled stress emitter — `npm run sim:udp-stress -- --rate 2000 --duration 10 --mix valid:50,garbage:50` — with exact sender-side counts to reconcile against the collector's drop counters; the agent sheds excess valid traffic as `backpressure` rather than queueing it, so a flood costs display lines, never memory.
