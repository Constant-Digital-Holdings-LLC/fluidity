...
//device-side signing, spelled out: set the MAC flag, hash the whole struct
//(flags included), append the trailer - exactly what firmware does
const signManually = (struct: Buffer, key: Uint8Array): Buffer => {
...
void test('udpCodec: verifyMac receives the exact signed-region/trailer split', () => {
    const wire = signManually(
        encodeFluPacket({ site: 's', plugin: 'p', fields: [{ style: 0, text: 'x' }] }),
        SIGNED_KEY
    );

    let seen: { signed: Buffer; mac: Buffer } | undefined;
...
void test('udpCodec: bad-mac masks downstream reasons (§6 step 5 before step 6)', () => {
    //full struct + trailer with a corrupted field_count: untrusted bytes
    //must surface as bad-mac, not as the count complaint
...
void test('udpCodec: real SipHash round-trip - genuine trailer passes, any flipped byte drops', () => {
    ...
    const verifyMac = (signed: Buffer, mac: Buffer): boolean => macEqual(siphash24(SIGNED_KEY, signed), mac);
    ...
    //tamper anywhere - payload, flags, seq, or the trailer itself
    for (const at of [5, 6, 14, FLU_HEADER_BYTES + 4, wire.length - 1]) {
        ...
        //most flips fail the MAC; flag/length-adjacent flips may trip
        //earlier validation - either way nothing tampered gets through
    }
