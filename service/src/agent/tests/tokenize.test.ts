...
const cfg = parseTokenizeConfig(undefined, true, 't'); //auto, on
const tok = (line: string): ReturnType<typeof tokenize> => tokenize(line, cfg);

//every emitted field must pass the server's own validation, or the agent
//would post a packet the server 400s
const allValid = (fields: ReturnType<typeof tokenize>): boolean => fields.every(isFormattedData);
...
void test('level words map to the palette anchors', () => {
    const styleOf = (lvl: string): number | undefined => {
...
void test('a URL bearing a control char stays a STRING, never an invalid LINK', () => {
    //URL_RE would match it, but the server rejects a LINK with control bytes;
    //the tokenizer must downgrade it to STRING so the packet still validates
...
void test('user rules win, first match, whole-line', () => {
    ...
    //a line not matching any rule falls through to normal tokenization
