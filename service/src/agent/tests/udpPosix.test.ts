...
import { startTarget, TestTarget } from './helpers.js';

//flu_udp_posix.h is the POSIX/Linux UDP transport a host C program uses to
//publish into the agent (the same-host story). These tests host-compile it
//(plus the worked example and a one-shot emitter) warning-free, then run the
//emitter to prove a real C program reaches a real udpStruct collector over UDP
//- open mode and MAC mode. Skipped without a C compiler or on Windows (the
//helper is POSIX-only; the agent runs on Linux).
...
let emit: string | undefined; //the one-shot emitter (MAC-enabled)
let exampleOk = false;
let compileErr = '';
...
if (cc && process.platform !== 'win32') {
    ...
    const e = join(buildDir, 'udpsend'); //udpsend.c #defines FLU_ENABLE_MAC itself
    const r1 = spawnSync(cc, [...common, '-o', e, join(FW, 'test', 'udpsend.c')], {
        encoding: 'utf8'
    });
    if (r1.status === 0) emit = e;
    else compileErr += r1.stderr;

    //the human-facing example must also stay warning-free (open mode, no MAC)
    const r2 = spawnSync(
        cc,
        [...common, '-o', join(buildDir, 'telemetry'), join(FW, 'examples', 'posix-telemetry.c')],
        {
            encoding: 'utf8'
        }
    );
    exampleOk = r2.status === 0;
    if (!exampleOk) compileErr += r2.stderr;
}
...
const collectorTo = (target: TestTarget, extendedOptions?: object): UdpStructCollector =>
...
void test('a C program publishes to the udpStruct collector over real UDP (open mode)', { skip }, async () => {
    ...
    try {
        ...
        const res = spawnSync(emit as string, ['127.0.0.1', String(port)], { encoding: 'utf8' });
...
void test('a C program signs (MAC mode) and a requireMac collector accepts it', { skip }, async () => {
    ...
    try {
        const port = await collector.ready();
        const res = spawnSync(emit as string, ['127.0.0.1', String(port), secret], { encoding: 'utf8' });
        assert.equal(res.status, 0, res.stderr);
