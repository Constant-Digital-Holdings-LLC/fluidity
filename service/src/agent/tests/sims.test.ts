...
const take = (gen: Generator<TimedLine, never, unknown>, n: number): TimedLine[] =>
    Array.from({ length: n }, () => gen.next().value);

const frameBytes = (line: string): number[] =>
    line
        .slice(1, -1)
        .split(' ')
        .map(b => parseInt(b, 16));
...
void test('srs port-state frames are a constant signature on a 100s heartbeat', () => {
    ...
    let now = 0;
    const portFrameTimes: number[] = [];
    const portFrameValues = new Set<string>();
    ...
    //invariants observed in production and implied by the docs
...
void test('srs radio-state frames model single-port COR with release-to-zero', () => {
    ...
    radio.forEach(line => {
        ...
        //single-port COR events only (matches f-y.io captures)
        assert.ok((cor & (cor - 1)) === 0, `multi-port COR unrealistic: ${line}`);
        //qualified receive only ever accompanies COR on the same port
        assert.equal(rcv & ~cor, 0, `RCVACT without COR: ${line}`);
        //PL/DTMF/PTT not modeled (always 00 in observed traffic)
        assert.equal(pl, 0);
        assert.equal(dtmf, 0);
        assert.equal(ptt, 0);
        //COR can't hop ports without releasing to zero in between
        if (cor && prevCor) {
...
void test('feeder writes optional banner then delimited lines from the source', async () => {
    const stub: SimProfile = {
        name: 'stub',
        banner: 'hello',
        delimiter: '\r\n',
        source: function* (): Generator<TimedLine, never, unknown> {
            for (;;) {
