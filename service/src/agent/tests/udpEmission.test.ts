...
//Functional coverage of the UDP emission path end to end, at small,
//loss-free rates: the stress emitter fires real datagrams at a real
//udpStruct collector, which posts over real HTTPS to a real makeApp server.
//This is the same harness `npm run loadtest` drives, exercised here so the
//shipped tool stays working and every emission category is pinned in CI.
//Rates are kept low (a few hundred/sec) so loopback never sheds and the
//accounting is exact.
...
void test('emission e2e: garbage is dropped at the agent, never forwarded', async () => {
    ...
    //random bytes fail the magic check first
...
void test('emission e2e: live packets fan out to SSE subscribers', async () => {
    ...
    //each of the 4 subscribers receives the broadcast, so frames >> posts
    assert.ok(
        r.sse.frames >= r.server.posts,
        `every post is fanned out (${r.sse.frames} frames, ${r.server.posts} posts)`
    );
