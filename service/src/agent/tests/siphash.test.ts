...
//the official SipHash-2-4 64-bit reference vectors (veorq/SipHash
//vectors_sip64): key = 000102...0f, message i = bytes 00..i-1, output
//bytes in wire order (little-endian u64) - exactly the MAC trailer layout
...
void test('siphash24 output depends on every message byte and on length', () => {
    const msg = Uint8Array.from({ length: 237 }, (_, i) => (i * 31) & 0xff); //max datagram size
...
void test('siphash24 reads subarray views correctly (Buffer pool offsets)', () => {
    //Buffers from the shared pool have nonzero byteOffset; the DataView
    //arithmetic must respect it or MACs of subarrays go quietly wrong
...
void test('sipKeyFromHex parses 32 hex chars and rejects everything else', () => {
    const key = sipKeyFromHex('000102030405060708090a0B0c0D0e0F'); //mixed case ok
    assert.ok(key);
...
void test('siphash24 throughput is far beyond the 1k datagrams/s requirement', () => {
    //time-bound catastrophe guard, box-relative so it survives slow/shared CI
    //cores while still catching a real regression: the allocation-free limb
    //impl clears 50k MACs in well under a second; the prior BigInt version
    //would blow the budget here (a CI core measured it ~8x slower). Warm the
    //JIT first so the logged rate reflects steady state, not cold code.
    ...
    //50k MACs in <4s is ~12k/s, still 12x the requirement; BigInt would miss it
