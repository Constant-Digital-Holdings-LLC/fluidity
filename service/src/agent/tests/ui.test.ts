...
void test('typewriter no-ops on a mobile-sized viewport, even with a frame source', async () => {
    ...
    //report a match for the max-width (mobile) query, not reduced-motion
    g['matchMedia'] = (q: string): { matches: boolean } => ({ matches: q.includes('max-width') });
    try {
        const root = dom.window.document.createElement('div');
        root.innerHTML = '<span>hello</span>';
        const frames: ((t: number) => void)[] = [];
        //a real frame source IS provided, so any no-op is the mobile gate alone
        ...
        if (orig === undefined) delete g['matchMedia'];
        else g['matchMedia'] = orig;
    }
});
...
void test('vRep heartbeats are presence, not content: site pill + version tooltip, no pane line, no collector pill', () => {
    ...
    const el = (id: string): HTMLElement | null => d.window.document.getElementById(id);
    const hb = (seq: number): FluidityPacket => pkt(seq, 'hub-agent', 'vRep', [str('Fluidity Agent 9.9.9', 10)]);

    //a history where the heartbeat is the site's ONLY traffic (the quiet-
    //gateway case): the site must still get a pill, but no pane line
    ...
    //presence surfaces on the site pill: created, alive, version in the title
    ...
    //live heartbeats behave the same: no line, liveness stays fresh
    u.packetAdd(hb(3));
...
void test('the Storing stat counts exactly the stored packets - 0 when the FIFO is empty', () => {
    ...
    const count = (): string => d.window.document.getElementById('visibile-count')?.textContent ?? '';

    //an empty FIFO stores nothing (a stray +1 used to render this as "1")
    new FluidityUI([]);
    assert.equal(count(), '0');

    //three history packets store as exactly three
    new FluidityUI([
