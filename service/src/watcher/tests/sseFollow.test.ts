...
const packet = (site: string, n: number): FluidityPacket => ({
    site,
    plugin: 'p',
...
const startServer = async (): Promise<{ port: number; drop: () => void; close: () => Promise<void> }> => {
    const conf = {
        appName: 'T',
        appVersion: 't',
    ...
    //server.close() waits for existing connections to drain - but an SSE
    //connection never drains on its own, so force sockets shut to avoid hangs
    return {
...
const post = (port: number, p: FluidityPacket): Promise<void> =>
...
void test('sseFollow reconciles /FIFO then streams live packets (deduped), and reports connection state', async () => {
    ...
    //two packets already in the FIFO before we subscribe
    await post(srv.port, packet('a', 1));
    await post(srv.port, packet('b', 2));

    const reconciled: FluidityPacket[][] = [];
    const live: FluidityPacket[] = [];
    let connected = 0;
    let disconnected = 0;

    const handle = follow(
        new URL(`https://127.0.0.1:${srv.port}`),
        { insecure: true },
        {
    ...
    try {
        ...
        //a new packet arrives live over SSE
...
void test('sseFollow reports disconnect when the server goes away', async () => {
    ...
    const handle = follow(
        new URL(`https://127.0.0.1:${srv.port}`),
        { insecure: true, backoffBaseMs: 50, backoffMaxMs: 100 },
        {
    ...
    try {
        ...
        srv.drop(); //server drops the live socket -> the SSE stream breaks
