...
const pkt = (over: Partial<FluidityPacket> = {}): FluidityPacket => ({
    site: 'greenhouse',
    plugin: 'logTail',
...
void test('parseDuration accepts ms/s/m/h and rejects junk', () => {
    ...
    assert.equal(parseDuration(250, 'x'), 250); //already ms
...
void test('matchesSelector: site/plugin equality + text regex (AND)', () => {
    ...
    //AND: all present fields must match
...
void test('parseRules: checkExec is invoked per enabled rule', () => {
    ...
    parseRules(
        [{ name: 'a', match: { site: 's' }, trigger: { type: 'silence', window: '1m' }, exec: '/usr/bin/foo' }],
        {
            checkExec: p => checked.push(p)
        }
    );
    assert.deepEqual(checked, ['/usr/bin/foo']);
    //a throwing checkExec propagates (startup fails loudly)
    assert.throws(
        () =>
            parseRules([{ name: 'a', match: { site: 's' }, trigger: { type: 'silence', window: '1m' }, exec: '/no' }], {
                checkExec: () => {
...
void test('parseRules: misconfiguration throws at parse time', () => {
    ...
    assert.throws(
        () => parseRules([{ ...base, trigger: { type: 'nope' } }]),
        /must be "match", "silence" or "frequency"/
    );
    assert.throws(
        () => parseRules([{ ...base, trigger: { type: 'frequency', count: 0, window: '1m' } }]),
