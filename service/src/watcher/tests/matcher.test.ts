...
void test('match fires on every matching packet, even a sustained stream', () => {
    ...
    m.setConnected(true);
    let t = 1_000_000;
    //packets closer together than any plausible window: every one fires
    ...
    //selector still gates: wrong site / non-matching text fire nothing
    m.observe(pkt('other', (t += 200), '[P5] x'), t);
    m.observe(pkt('s', (t += 200), '[P3] info'), t);
    assert.equal(fires.length, 3);
    //evaluate() is a no-op for match rules
    m.evaluate(t + 60_000);
    assert.equal(fires.length, 3);
});
