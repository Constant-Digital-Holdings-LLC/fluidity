...
const params = (notifyIntervalSec = 900) => ({
    plugin: 'hamLive',
...
void test('an upcoming net includes its computed start time', () => {
    const c = new HamLiveCollector(params());
    const out = c.format(JSON.stringify({
        netlist: [
            net({
                id: 'n-upcoming',
                started: false,
...
void test('permanent nets are filtered out and repeat notifications are throttled', () => {
    ...
    const permanentOnly = c.format(JSON.stringify({ netlist: [net({ id: 'n-perm', permanent: true })] }), new FormatHelper());
...
void test('a real captured ham.live response parses without error', () => {
    const fixturePath = fileURLToPath(new URL('../../../../sims/fixtures/ham-live-livenets-2026-06-11.json', import.meta.url));
