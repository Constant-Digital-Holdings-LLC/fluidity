...
void test('C firmware datagrams are byte-identical to both TS implementations', { skip }, () => {
    ...
    assert.ok(wire.get('open-full')?.equals(encodeFluPacket({ ...greenhouse, full: true })), 'C vs agent codec (full struct)');
    assert.ok(wire.get('timestamped')?.equals(encodeFluPacket({ ...greenhouse, tsEpochSec: 1_765_000_000 })), 'C vs agent codec (FLU_F_TS)');
});
