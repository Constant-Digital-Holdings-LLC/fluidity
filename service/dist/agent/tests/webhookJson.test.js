...
import WebhookJsonCollector from '../modules/collectors/webhookJson.js';
...
void test('webhookJson maps a Kuma DOWN webhook into a routed FluidityPacket end to end', async () => {
    ...
    try {
        ...
        assert.deepEqual(packet.formattedData.map(f => f.field), ['[P5]', '[Plex] [Down] connect ECONNREFUSED']);
...
void test('webhookJson value mapping: UP, unmapped status, and an absent path take map/default/omit', async () => {
    ...
    try {
        ...
        let p = (await next);
        assert.deepEqual(p.formattedData.map(f => [f.field, f.suggestStyle]), [
            ['[P4]', 3],
            ['up again', 0]
        ]);
        next = target.next();
        ...
        assert.equal(p.formattedData[0]?.suggestStyle, 0);
        assert.equal(p.description, 'Uptime Kuma');
        next = target.next();
        await post('/kuma', { heartbeat: null, monitor: null, msg: 'Testing' });
        p = (await next);
        assert.deepEqual(p.formattedData.map(f => f.field), ['[P3]', 'Testing']);
    }
    finally {
        close();
        target.server.close();
...
void test('webhookJson: absent value with no default omits the field; all-absent answers ok-empty and posts nothing', async () => {
    ...
    try {
        ...
        const p = (await next);
        assert.deepEqual(p.formattedData.map(f => f.field), ['[P5]']);
...
void test('webhookJson extraction: const fields, object dumps, and array index paths', async () => {
    ...
    try {
        ...
        const p = (await next);
        assert.deepEqual(p.formattedData.map(f => f.field), ['static-lead', '{"a":1}', 'one', 'false']);
...
void test('webhookJson misconfiguration throws at startup (degrade loudly, never run weakened)', () => {
    ...
    const routes = (r) => ({ routes: r });
    const cases = [
        [{ port: 70000 }, /port must be an integer/],
        [{ bind: 9 }, /bind must be an interface address/],
