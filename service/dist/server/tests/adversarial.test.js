...
const post = (base, body, key = API_KEY) => fetch(`${base}/FIFO`, {
    method: 'POST',
    headers: {
...
const fifo = async (base) => (await (await fetch(`${base}/FIFO`, { signal: AbortSignal.timeout(3000) })).json());
void test('a rejected POST advances neither seq nor the FIFO (no burned numbers, no leak)', async () => {
    ...
    try {
        ...
        assert.deepEqual(arr.map(p => p.site), ['first', 'second']);
        assert.deepEqual(arr.map(p => p.seq), [1, 2], 'rejected POSTs never advanced the seq counter');
    }
    finally {
        close();
    }
...
void test('seq is gap-free and monotonic across many sequential posts', async () => {
    ...
    try {
        const N = 30;
        for (let i = 0; i < N; i++)
            assert.equal((await post(base, valid({ site: `s${i}` }))).status, 200);
        const arr = await fifo(base);
        assert.deepEqual(arr.map(p => p.seq), Array.from({ length: N }, (_, i) => i + 1));
    }
    finally {
        close();
    }
...
void test('concurrent posts produce a unique, gap-free seq set (no double-assignment)', async () => {
    ...
    try {
        ...
        assert.deepEqual(seqs, Array.from({ length: N }, (_, i) => i + 1), 'every concurrent post got a distinct seq, 1..N with no gaps or repeats');
    }
    finally {
        close();
    }
...
void test('the live SSE id equals the broadcast packet seq', async () => {
    ...
    try {
        ...
        let buf = '';
        const hasBlock = (b) => b.includes('id:') && b.includes('data:') && b.includes('\n\n');
        while (!hasBlock(buf)) {
